// src/models/activityDetection.js

export const runActivityDetection = async (webcamRef, setDetections) => {
  if (!webcamRef.current || !webcamRef.current.video || webcamRef.current.video.readyState !== 4) {
    console.log("Webcam not ready for activity detection.");
    return;
  }
  ...
  try {
    ...
    if (data && data.data && data.data.activities) {
      setDetections(data.data.activities);
    } else if (data && data.error) {
      setDetections([{ error: data.error }]);
    } else {
      setDetections([{ message: "No activity data received." }]);
    }
  } catch (error) {
